...
console.log("Request to ", PROXY_ADDRESS);
...
// 함수 선언부
async function mapPostListToImageList(postList) {
  return await Promise.all(
    postList.map(async ({ url: postUrl, author }) => {
      const {
        data: { base64Source },
      } = await axios(`${PROXY_ADDRESS}/instagram/postImage`, {
        params: {
          postUrl,
        },
      });
...
function renderImageList(list) {
  for (let i = 0; i < list.length; i++) {
    const { author, base64Source } = list[i];
